...
## Section list rules (only when should_fanout=true)
  ...
  - If the natural shape genuinely exceeds {max_sections}, COLLAPSE semantically-adjacent ones into combined sections (e.g. "Facts and Background", "Verification and Affidavit", "Cause Title and Parties") so the list fits. This ceiling is BUDGET-DRIVEN: sections are written two per Gemini Pro call (~25s each), so a list near the ceiling costs roughly ten sequential calls and approaches the request timeout on live traffic — plans that large should be genuinely rare. A full writ petition, plaint or written statement fits comfortably below it once adjacent overlaps are merged. If you do plan more than {max_sections}, the downstream trim keeps the FIRST sections and the LAST one — middle sections are dropped silently, so it is your job to compress at plan time rather than leave the choice to the trim.
  ...
  - The LAST section is MANDATORY and must be the block the document actually ENDS with — the one that makes it executable rather than a fragment. For PLEADINGS that is the Prayer / Verification / affidavit block. For TRANSACTIONAL INSTRUMENTS (MOU, agreement, deed, lease, contract, undertaking) it is the execution block — `IN WITNESS WHEREOF ...`, the parties' signature lines, witnesses, and any Schedule / Annexure the body refers to. NEVER end the plan on a substantive clause such as Governing Law, Dispute Resolution, Entire Agreement or Miscellaneous: a contract that stops after its governing-law clause is an unsigned fragment the advocate can neither file nor execute.
