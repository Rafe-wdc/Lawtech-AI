...
# the orchestrator to silently fall back to the Judgment-only synthesis path
# (BUG-02 manifested as a case-law summary instead of a draft).
#
# This hook runs once per worker after fork. It primes the ES connection
# pool with a no-op ping and warms the Gemini Flash channel with a
# minimal token-cheap call. Each warm-up step is wrapped in try/except so
# a transient outage in one dependency cannot wedge worker startup.
def post_worker_init(worker):  # pragma: no cover  — gunicorn-only hook
    import logging
    log = logging.getLogger("gunicorn.error")
    log.info("[warmup] worker pid=%s starting", worker.pid)
    ...
    # 4. Embeddings — HF model is already in-memory via preload_app, but
    #    a single embed_query primes any lazy CUDA/MPS state and confirms
    #    health.
    try:
    ...
    log.info("[warmup] worker pid=%s ready", worker.pid)
