...
# ---------------------------------------------------------------------------
# Label-only sources are not citations.
#
# Lawyer-reported answer (2026-09-17): a case-lookup follow-up ended with
#   ## Additional Analysis
#   - AI-Generated Legal Analysis
# Scenario's prose was dropped as redundant, the append branch fell back to its
# structured sources, and the only one was the placeholder Scenario attaches
# when web grounding returns no pages.
# ---------------------------------------------------------------------------

def test_placeholder_only_supporter_adds_no_heading_or_bullet():
    import asyncio
    from core.state import AgentResult, SourceMetadata
    from agents.orchestrator import orchestrator_synthesize_node

    primary = (
        "The Supreme Court held that expert evidence is not mandatory in every "
        "medical negligence complaint where negligence is evident on the record.\n\n"
        "### V. Kishan Rao v. Nikhil Super Speciality Hospital, (2010) 5 SCC 513\n"
        "Res ipsa loquitur shifts the burden to the hospital.\n\n"
        "## PDF Links\n- [V. Kishan Rao](https://api.sci.gov.in/supremecourt/kishan.pdf)\n"
    )
    state = {
        "original_query": "is expert opinion mandatory in medical negligence",
        "query": "is expert opinion mandatory in medical negligence",
        "task": "SCI_Judgment",
        "tasks_planned": ["SCI_Judgment", "Scenario"],
        "previous_artifact_content": "",
        "agent_results": {
            "SCI_Judgment": AgentResult(agent_name="SCI_Judgment", content=primary),
            # Scenario restates the primary, so every block is redundant.
            "Scenario": AgentResult(
                agent_name="Scenario", content=primary,
                sources=[SourceMetadata(
                    source_type="scenario", title="AI-Generated Legal Analysis",
    ...
    answer = asyncio.run(orchestrator_synthesize_node(state))["final_response"]
    assert "AI-Generated Legal Analysis" not in answer
    assert "## Additional Analysis" not in answer
    assert primary.rstrip() in answer


def test_citation_list_keeps_judgments_and_statutes_but_not_labels():
    from core.state import AgentResult, SourceMetadata
    from agents.orchestrator import _sources_as_citation_list

    result = AgentResult(agent_name="X", content="", sources=[
        SourceMetadata(source_type="scenario", title="AI-Generated Legal Analysis"),
        SourceMetadata(source_type="judgment", title="Sarabjit Kaur v. State of Punjab",
                       court_name="Supreme Court of India", year=2023),
        SourceMetadata(source_type="newacts", title="The Bharatiya Nyaya Sanhita - Section 318",
                       act_name="The Bharatiya Nyaya Sanhita, 2023", section_number="318"),
        SourceMetadata(source_type="judgment", title="Ramdeo Prasad Singh vs Dr. Ramesh Chandra Sinha",
                       doc_link="https://lawttorney.s3.ap-south-1.amazonaws.com/tribunal/x.pdf"),
    ])
    rendered = _sources_as_citation_list(result)
    assert "AI-Generated Legal Analysis" not in rendered
