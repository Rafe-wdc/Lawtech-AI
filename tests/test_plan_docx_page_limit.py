...
and counts those pages, else lets the file through. The converter is
simulated here.
"""
...
def test_docx_over_30_pages_is_rejected(client, converter, plan):
    converter["pages"] = 31
    r = _post(client, _docx(), plan)
    assert r.status_code == 403
    assert r.json()["message"] == (
        f"notice.docx has 31 pages. Your plan ({plan}) allows up to 30 pages per document"
    )


def test_docx_at_30_pages_is_accepted(client, converter):
    converter["pages"] = 30
...
def test_page_count_saved_by_word_is_used_without_converting(client, converter):
    ...
    assert _post(client, _docx(saved_pages=40)).status_code == 403
...
def test_page_count_unknown_lets_the_file_through(client, converter):
