...
| _(none)_ | — | [`core/retrieval_relevance.py`](../core/retrieval_relevance.py) | Relevance gate is a single Gemini Flash Lite call returning `relevant`/`reason`. Prior `RETRIEVAL_GATE_COARSE_FLOOR` (coarse embedding pre-filter) and `RETRIEVAL_GATE_MIN_CONFIDENCE` (judge confidence threshold) were removed. |
