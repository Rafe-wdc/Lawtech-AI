"""Reproduce the client's exact counter-arguments bug WITH the attached files.

Client's setup:
- prompt: "Make notes of counter arguments on behalf of Opponent, Cover all the
  points including Violation of 33(2)(b)..."
- files:  Written Notes of Argument Speciality (1).pdf   (~13MB, PDF)
          SPECIALITY-Workman_Arguments_Section_33_2b-2.docx  (~34KB, docx)

The docx filename literally says "Workman_Arguments" — likely contains the
workman's arguments. The concern: the pipeline reads that as source material
and echoes the workman's arguments back instead of countering them.

Target: env SMOKE_BASE (defaults to localhost).
Uses /pyapi/chat (multipart) which is the correct endpoint for file attachments.
"""
...
FILE_1 = Path(r"D:\agentic_proj\Lawtech-AI\test_pdfs\Written Notes of Argument Speciality (1).pdf")
FILE_2 = Path(r"D:\agentic_proj\Lawtech-AI\test_docx\SPECIALITY-Workman_Arguments_Section_33_2b-2.docx")

OUT_DIR = Path(__file__).parent
OUT_MD = OUT_DIR / f"_counter_args_files_{TAG}.md"
OUT_META = OUT_DIR / f"_counter_args_files_{TAG}_meta.json"
OUT_RAW = OUT_DIR / f"_counter_args_files_{TAG}_events.jsonl"


async def main() -> int:
    if not FILE_1.exists() or not FILE_2.exists():
        print(f"MISSING: {FILE_1.exists()=} {FILE_2.exists()=}")
        return 2
    body1 = FILE_1.read_bytes()
    body2 = FILE_2.read_bytes()
    print(f"BASE: {BASE}")
    print(f"FILE1: {FILE_1.name} ({len(body1):,} bytes)")
    print(f"FILE2: {FILE_2.name} ({len(body2):,} bytes)")
    print(f"PROMPT ({len(PROMPT)} chars): {PROMPT[:100]}...")

    t0 = time.time()
    data = {"query": PROMPT}
    files = [
        ("files", (FILE_1.name, body1, "application/pdf")),
        ("files", (FILE_2.name, body2,
        ...
        async with httpx.AsyncClient() as client:
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    print(f"HTTP {resp.status_code}: {err.decode(errors='replace')[:600]}")
                    return 1
    ...
    meta = {
        ...
        "file_processing_summary": [
            {"message": e.get("message"), "files": e.get("files"),
             "stage": e.get("stage"), "rejected": e.get("rejected")}
